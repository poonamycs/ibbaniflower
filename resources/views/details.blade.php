...
<div class="product-detail">
								...
								<div class="prince"><span>&#8377;&nbsp;{{$product->price}}</span><s class="strike">&#8377;{{$product->discount}}</s></div>
									...
									<h6><i class="fa fa-shopping-bag"></i> Check product availability in your area</h6>
										<div class="form-inline pincode-search">
											...
											<div id="confirmation">
												<span id="slotdetails{{$product->id}}"></span>
											</div>
								...
								<div class="col-lg-7 col-md-6 col-sm-12 col-xs-12 detail">
									...
									<div class="prince"><span>&#8377;&nbsp;{{ $product->price }}</span><s class="strike">&#8377;{{$product->discount}}</s></div>
										...
										<div class="form-inline pincode-search">
											...
											<div id="confirmation">
												<span id="slotdetails{{$product_img->id}}"></span>
											</div>
	...
	<input type="hidden" id="confirm_data_id" value="">
...
<script>
	...
	$(".selectedSlot1").change(function() {
				...
				if(shipping_date.length > 0)
				{
					$.ajax({
						...
						success:function(resp){	
							...
							if(resp){
							for (i = 0; i < resp.length; ++i) {
									option += ('<option value="' + i + '">' + resp[i]['slot'] + '</option>');
									console.log(option);
								}
								option += '</select>';
								console.log("hi");
        ...
        function confirmSlot() {
			...
			var data_id = $("#confirm_data_id").val();
			alert(date);
			alert(slot);
			alert(time);
           document.getElementById('confirmation').style.display = 'block';

            var slotdetails = document.getElementById('slotdetails'+data_id);
