...
class BlogsController extends Controller
{
    	...
    	if($request->isMethod('post')){
    		...
    		if($request->hasFile('image')){
    			...
    			if($image_tmp->isValid()){
    				...
    				$large_image_path = 'images/backend_images/blogs/large/'.$filename;
    				$medium_image_path = 'images/backend_images/blogs/medium/'.$filename;
    ...
    public function editBlog(Request $request, $pid){
    ...
    public function blogApproved(Request $request, $id = null){
        if($request->isMethod('post')){
            $data = $request->all();
            if(empty($data['status'])){
                $status='0';
    ...
    public function viewAdminBlogs(){
        $blogs = Blog::where('status',1)->get();
        return view('admin.blogs.view_all_blogs')->with(compact('blogs'));
    }
